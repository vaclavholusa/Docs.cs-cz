...
<span data-ttu-id="15c66-174">V této úloze bude projít kroky povolení **migrace Entity Framework Code First** k **kvíz Informatik** databáze, změna modelu a vysvětlení, jak se tyto změny projeví v databáze.</span><span class="sxs-lookup"><span data-stu-id="15c66-174">In this task, you will go through the steps of enabling **Entity Framework Code First Migrations** to the **Geek Quiz** database, changing the model and understanding how those changes are reflected in the database.</span></span>
...
11. <span data-ttu-id="15c66-211">Otevřít **GeekQuizProd** databáze a rozbalte **tabulky** uzlu.</span><span class="sxs-lookup"><span data-stu-id="15c66-211">Open the **GeekQuizProd** database and expand the **Tables** node.</span></span> <span data-ttu-id="15c66-212">Jak je vidět **aktualizace databáze** příkaz vygeneruje všechny tabulky, které jsou definovány v **TriviaContext** třídy.</span><span class="sxs-lookup"><span data-stu-id="15c66-212">As you can see, the **Update-Database** command generated all the tables defined in the **TriviaContext** class.</span></span> <span data-ttu-id="15c66-213">Vyhledejte **dbo. TriviaQuestions** tabulky a otevřete uzel sloupce.</span><span class="sxs-lookup"><span data-stu-id="15c66-213">Locate the **dbo.TriviaQuestions** table and open the columns node.</span></span> <span data-ttu-id="15c66-214">V dalším úkolem, přidáte nový sloupec do této tabulky a aktualizujte databáze s využitím **migrace**.</span><span class="sxs-lookup"><span data-stu-id="15c66-214">In the next task, you will add a new column to this table and update the database using **Migrations**.</span></span>
    ...
    <span data-ttu-id="15c66-216">*Triviální prvek otázky, které sloupce*</span><span class="sxs-lookup"><span data-stu-id="15c66-216">*Trivia Questions Columns*</span></span>
...
3. <span data-ttu-id="15c66-313">Kopírovat **připojovací řetězec** hodnotu a zavřete dialogové okno.</span><span class="sxs-lookup"><span data-stu-id="15c66-313">Copy the **connection string** value and close the dialog box.</span></span>
    ...
    <span data-ttu-id="15c66-315">*Připojovací řetězec v portálu pro správu Azure*</span><span class="sxs-lookup"><span data-stu-id="15c66-315">*Connection String in Azure Management Portal*</span></span>
...
2. <span data-ttu-id="15c66-339">Přejděte **škálování** stránky.</span><span class="sxs-lookup"><span data-stu-id="15c66-339">Navigate to the **Scale** page.</span></span> <span data-ttu-id="15c66-340">V části **Obecné** vyberte **standardní** konfigurace a klikněte na **Uložit** na panelu příkazů.</span><span class="sxs-lookup"><span data-stu-id="15c66-340">Under the **general** section, select **Standard** for the configuration and click **Save** in the command bar.</span></span>
    ...
    <span data-ttu-id="15c66-344">*Upgrade webové aplikace na standardní režim*</span><span class="sxs-lookup"><span data-stu-id="15c66-344">*Upgrading the Web App to Standard mode*</span></span>
...
12. <span data-ttu-id="15c66-381">Klikněte na tlačítko **nasazení** zobrazíte **historie nasazení**.</span><span class="sxs-lookup"><span data-stu-id="15c66-381">Click **Deployments** to see the **deployment history**.</span></span> <span data-ttu-id="15c66-382">Ověřte, zda je **aktivní nasazení** s vaší  *&quot;počáteční potvrzení&quot;*.</span><span class="sxs-lookup"><span data-stu-id="15c66-382">Verify that there is an **Active Deployment** with your *&quot;Initial Commit&quot;*.</span></span>
...
<span data-ttu-id="15c66-420">Existují scénáře, ve kterém nemáte přípravný slot provádět horké prohození mezi přípravným a produkčním prostředím, například pokud pracujete s **Free** nebo **Shared** režimu.</span><span class="sxs-lookup"><span data-stu-id="15c66-420">There are scenarios where you do not have a staging slot to perform hot swap between staging and production, for example, if you are working with **Free** or **Shared** mode.</span></span> <span data-ttu-id="15c66-421">V těchto scénářích měli byste otestovat vaši aplikaci v testovacím prostředí – místně nebo ve vzdálené lokalitě – před nasazením do produkčního prostředí.</span><span class="sxs-lookup"><span data-stu-id="15c66-421">In those scenarios, you should test your application in a testing environment –either locally or in a remote site– before deploying to production.</span></span> <span data-ttu-id="15c66-422">Je však možné, že problém nebyl zjištěn během fáze testování mohou nastat v produkční lokality.</span><span class="sxs-lookup"><span data-stu-id="15c66-422">However, it is possible that an issue not detected during the testing phase may arise in the production site.</span></span> <span data-ttu-id="15c66-423">V takovém případě je důležité mít mechanismus pro snadno přepínat na předchozí a více stabilní verzi aplikace co nejrychleji.</span><span class="sxs-lookup"><span data-stu-id="15c66-423">In this case, it is important to have a mechanism to easily switch to a previous and more stable version of the application as quickly as possible.</span></span>

<span data-ttu-id="15c66-424">V **služby Azure App Service**, toto je to možné díky díky průběžné nasazování ze správy zdrojového kódu **znovu nasadit** akce, které jsou k dispozici na portálu management portal.</span><span class="sxs-lookup"><span data-stu-id="15c66-424">In **Azure App Service**, continuous deployment from source control makes this possible thanks to the **redeploy** action available in the management portal.</span></span> <span data-ttu-id="15c66-425">Uchovává informace o nasazení přidružená k potvrzení změn do úložiště Azure a nabízí možnost znovu nasadit aplikaci pomocí kteréhokoli z předchozích nasazení, kdykoli.</span><span class="sxs-lookup"><span data-stu-id="15c66-425">Azure keeps track of the deployments associated with the commits pushed to the repository and provides an option to redeploy your application using any of your previous deployments, at any time.</span></span>

<span data-ttu-id="15c66-426">V tomto cvičení budete provádět změny kódu v **Informatik kvíz** aplikaci, která záměrně vkládá *chyb*.</span><span class="sxs-lookup"><span data-stu-id="15c66-426">In this exercise you will perform a change to the code in the **Geek Quiz** application that intentionally injects a *bug*.</span></span> <span data-ttu-id="15c66-427">Budete nasazovat aplikaci do produkčního prostředí, abyste viděli chybu a pak bude využít výhod funkce opětovného nasazení se vrátíte do předchozího stavu.</span><span class="sxs-lookup"><span data-stu-id="15c66-427">You will deploy the application to production to see the error, and then you will take advantage of the redeploy feature to go back to the previous state.</span></span>

<a id="Ex3Task1"></a>
#### <a name="task-1--updating-the-geek-quiz-application"></a><span data-ttu-id="15c66-428">Úloha 1 – aktualizace aplikace kvíz Informatik</span><span class="sxs-lookup"><span data-stu-id="15c66-428">Task 1 – Updating the Geek Quiz Application</span></span>

<span data-ttu-id="15c66-429">V této úloze bude Refaktorovat malou část kódu **TriviaController** třídy k extrakci část logiky, který načte kvíz vybranou možnost z databáze do nové metody.</span><span class="sxs-lookup"><span data-stu-id="15c66-429">In this task, you will refactor a small piece of code of the **TriviaController** class to extract part of the logic that retrieves the selected quiz option from the database into a new method.</span></span>

1. <span data-ttu-id="15c66-430">Přepnout na instanci aplikace Visual Studio s **GeekQuiz** řešení v předchozím cvičení.</span><span class="sxs-lookup"><span data-stu-id="15c66-430">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="15c66-431">V **Průzkumníka řešení**, otevřete **TriviaController.cs** soubor uvnitř **řadiče** složky.</span><span class="sxs-lookup"><span data-stu-id="15c66-431">In **Solution Explorer**, open the **TriviaController.cs** file inside the **Controllers** folder.</span></span>
3. <span data-ttu-id="15c66-432">Vyhledejte **StoreAsync** metody a vyberte zvýrazněný kód na následujícím obrázku.</span><span class="sxs-lookup"><span data-stu-id="15c66-432">Locate the **StoreAsync** method and select the code highlighted in the following figure.</span></span>

    ![Vyberte kód](maintainable-azure-websites-managing-change-and-scale/_static/image47.png)

    <span data-ttu-id="15c66-434">*Vyberte kód*</span><span class="sxs-lookup"><span data-stu-id="15c66-434">*Selecting the code*</span></span>
4. <span data-ttu-id="15c66-435">Klikněte pravým tlačítkem na vybraný kód, rozbalte **Refaktorovat** nabídky a vybereme **extrahovat metodu...** .</span><span class="sxs-lookup"><span data-stu-id="15c66-435">Right-click the selected code, expand the **Refactor** menu and select **Extract Method...**.</span></span>

    ![Extrahování kód jako novou metodu](maintainable-azure-websites-managing-change-and-scale/_static/image48.png)

    <span data-ttu-id="15c66-437">*Výběr metody extrahování*</span><span class="sxs-lookup"><span data-stu-id="15c66-437">*Selecting Extract Method*</span></span>
5. <span data-ttu-id="15c66-438">V **extrahovat metodu** dialogové okno, název nové metody *MatchesOption* a klikněte na tlačítko **OK**.</span><span class="sxs-lookup"><span data-stu-id="15c66-438">In the **Extract Method** dialog box, name the new method *MatchesOption* and click **OK**.</span></span>

    ![Zadání názvu – metoda](maintainable-azure-websites-managing-change-and-scale/_static/image49.png)

    <span data-ttu-id="15c66-440">*Zadejte název pro extrahovaný – metoda*</span><span class="sxs-lookup"><span data-stu-id="15c66-440">*Specifying the name for the extracted method*</span></span>
6. <span data-ttu-id="15c66-441">Vybraný kód je pak extrahován do **MatchesOption** metody.</span><span class="sxs-lookup"><span data-stu-id="15c66-441">The selected code is then extracted into the **MatchesOption** method.</span></span> <span data-ttu-id="15c66-442">Výsledný kód je znázorněno v následujícím fragmentu kódu.</span><span class="sxs-lookup"><span data-stu-id="15c66-442">The resulting code is shown in the following snippet.</span></span>

    [!code-csharp[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample14.cs)]
7. <span data-ttu-id="15c66-443">Stisknutím klávesy **stisknutím CTRL + S** a uložte změny.</span><span class="sxs-lookup"><span data-stu-id="15c66-443">Press **CTRL + S** to save the changes.</span></span>

<a id="Ex3Task2"></a>
#### <a name="task-2--redeploying-the-geek-quiz-application"></a><span data-ttu-id="15c66-444">Úloha 2 – opětovného nasazení aplikace kvíz Informatik</span><span class="sxs-lookup"><span data-stu-id="15c66-444">Task 2 – Redeploying the Geek Quiz Application</span></span>

<span data-ttu-id="15c66-445">Nyní budou nabízená oznámení změn, které jste provedli v předchozí úloze úložiště, které aktivují nové nasazení do produkčního prostředí.</span><span class="sxs-lookup"><span data-stu-id="15c66-445">You will now push the changes you made in the previous task to the repository, which will trigger a new deployment to the production environment.</span></span> <span data-ttu-id="15c66-446">Potom se potíže, problém pomocí **vývojářské nástroje F12** poskytované Internet Exploreru a pak proveďte vrácení zpět na předchozí nasazení z portálu pro správu Azure.</span><span class="sxs-lookup"><span data-stu-id="15c66-446">Then, you will troubleshot an issue using the **F12 development tools** provided by Internet Explorer, and then perform a rollback to the previous deployment from the Azure management portal.</span></span>

1. <span data-ttu-id="15c66-447">Otevřete novou **Git Bash** konzoly nasaďte aktualizovanou aplikaci do služby Azure App Service.</span><span class="sxs-lookup"><span data-stu-id="15c66-447">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
2. <span data-ttu-id="15c66-448">Spusťte následující příkazy, které odešlete změny do Azure.</span><span class="sxs-lookup"><span data-stu-id="15c66-448">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="15c66-449">Aktualizace *[YOUR cesta aplikace]* zástupný symbol s cestou **GeekQuiz** řešení.</span><span class="sxs-lookup"><span data-stu-id="15c66-449">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="15c66-450">Zobrazí se výzva k zadání hesla nasazení.</span><span class="sxs-lookup"><span data-stu-id="15c66-450">You will be prompted for your deployment password.</span></span>
    ...
    <span data-ttu-id="15c66-452">*Nahráním refaktorovaný kódu do Azure*</span><span class="sxs-lookup"><span data-stu-id="15c66-452">*Pushing refactored code to Azure*</span></span>
3. <span data-ttu-id="15c66-453">Otevřete aplikaci Internet Explorer a přejděte do své webové aplikace (například `http://<your-web-site>.azurewebsites.net`).</span><span class="sxs-lookup"><span data-stu-id="15c66-453">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="15c66-454">Přihlaste se pomocí dříve vytvořeného přihlašovacích údajů.</span><span class="sxs-lookup"><span data-stu-id="15c66-454">Log in using the previously created credentials.</span></span>
4. <span data-ttu-id="15c66-455">Stisknutím klávesy **F12** ke spuštění nástroje pro vývoj, vyberte **sítě** kartě a klikněte na tlačítko **Přehrát** tlačítko Spustit záznam.</span><span class="sxs-lookup"><span data-stu-id="15c66-455">Press **F12** to launch the development tools, select the **Network** tab and click the **Play** button to start recording.</span></span>

    <span data-ttu-id="15c66-456">![Spouštění nahrávání sítě](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "od sítě záznam")</span><span class="sxs-lookup"><span data-stu-id="15c66-456">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "Starting network recording")</span></span>

    <span data-ttu-id="15c66-457">*Spouští se záznam sítě*</span><span class="sxs-lookup"><span data-stu-id="15c66-457">*Starting network recording*</span></span>
5. <span data-ttu-id="15c66-458">Vyberte všechny možnosti kvíz.</span><span class="sxs-lookup"><span data-stu-id="15c66-458">Select any option of the quiz.</span></span> <span data-ttu-id="15c66-459">Zobrazí se, že se nic nestane.</span><span class="sxs-lookup"><span data-stu-id="15c66-459">You will see that nothing happens.</span></span>
6. <span data-ttu-id="15c66-460">V **F12** okno, zobrazuje položku odpovídající požadavek POST HTTP protokolu HTTP **500** výsledek.</span><span class="sxs-lookup"><span data-stu-id="15c66-460">In the **F12** window, the entry corresponding to the POST HTTP request shows an HTTP **500** result.</span></span>

    ![HTTP 500 chyb](maintainable-azure-websites-managing-change-and-scale/_static/image52.png)

    <span data-ttu-id="15c66-462">*HTTP 500 chyb*</span><span class="sxs-lookup"><span data-stu-id="15c66-462">*HTTP 500 error*</span></span>
7. <span data-ttu-id="15c66-463">Vyberte **konzoly** kartu. Podrobnosti o příčině je zaznamenána chyba.</span><span class="sxs-lookup"><span data-stu-id="15c66-463">Select the **Console** tab. An error is logged with the details of the cause.</span></span>

    ![Zaznamenané chyby](maintainable-azure-websites-managing-change-and-scale/_static/image53.png)

    <span data-ttu-id="15c66-465">*Zaznamenané chyby*</span><span class="sxs-lookup"><span data-stu-id="15c66-465">*Logged error*</span></span>
8. <span data-ttu-id="15c66-466">Vyhledejte část Podrobnosti o této chybě.</span><span class="sxs-lookup"><span data-stu-id="15c66-466">Locate the details part of the error.</span></span> <span data-ttu-id="15c66-467">Je zřejmé tato chyba je způsobena kódu, refaktoring jste v předchozích krocích.</span><span class="sxs-lookup"><span data-stu-id="15c66-467">Clearly, this error is caused by the code refactoring you committed in the previous steps.</span></span>

    <span data-ttu-id="15c66-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span><span class="sxs-lookup"><span data-stu-id="15c66-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span></span>
9. <span data-ttu-id="15c66-469">Nezavírejte prohlížeče.</span><span class="sxs-lookup"><span data-stu-id="15c66-469">Do not close the browser.</span></span>
10. <span data-ttu-id="15c66-470">V nové instanci prohlížeče, přejděte [portál pro správu Azure](https://manage.windowsazure.com) a přihlaste se pomocí účtu Microsoft, který je spojen s vaším předplatným.</span><span class="sxs-lookup"><span data-stu-id="15c66-470">In a new browser instance, navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
11. <span data-ttu-id="15c66-471">Vyberte **Websites** a klikněte na webovou aplikaci, kterou jste vytvořili v cvičení 2.</span><span class="sxs-lookup"><span data-stu-id="15c66-471">Select **Websites** and click the web app you created in Exercise 2.</span></span>
12. <span data-ttu-id="15c66-472">Přejděte **nasazení** stránky.</span><span class="sxs-lookup"><span data-stu-id="15c66-472">Navigate to the **Deployments** page.</span></span> <span data-ttu-id="15c66-473">Všimněte si, že provádí všechna potvrzení změn jsou uvedeny v historii nasazení.</span><span class="sxs-lookup"><span data-stu-id="15c66-473">Notice that all the commits performed are listed in the deployment history.</span></span>

    ![Seznam existujících nasazení](maintainable-azure-websites-managing-change-and-scale/_static/image54.png)

    <span data-ttu-id="15c66-475">*Seznam existujících nasazení*</span><span class="sxs-lookup"><span data-stu-id="15c66-475">*List of existing deployments*</span></span>
13. <span data-ttu-id="15c66-476">Vybrat předchozí potvrzení změn a klikněte na tlačítko **znovu nasadit** na panelu příkazů.</span><span class="sxs-lookup"><span data-stu-id="15c66-476">Select the previous commit and click **Redeploy** on the command bar.</span></span>

    ![Znovu se nasazuje předchozí potvrzení změn](maintainable-azure-websites-managing-change-and-scale/_static/image55.png)

    <span data-ttu-id="15c66-478">*Znovu se nasazuje předchozí potvrzení změn*</span><span class="sxs-lookup"><span data-stu-id="15c66-478">*Redeploying the previous commit*</span></span>
14. <span data-ttu-id="15c66-479">Po zobrazení výzvy k potvrzení klikněte na tlačítko **Ano**.</span><span class="sxs-lookup"><span data-stu-id="15c66-479">When prompted to confirm, click **Yes**.</span></span>

    ![Potvrzují se opětovné nasazení](maintainable-azure-websites-managing-change-and-scale/_static/image56.png)
15. <span data-ttu-id="15c66-481">Po dokončení nasazení se přepněte zpět do instance prohlížeče s vaší webové aplikace a stisknutím klávesy **CTRL + F5**.</span><span class="sxs-lookup"><span data-stu-id="15c66-481">When the deployment completes, switch back to the browser instance with your web app and press **CTRL + F5**.</span></span>
16. <span data-ttu-id="15c66-482">Klikněte na některou z možností.</span><span class="sxs-lookup"><span data-stu-id="15c66-482">Click any of the options.</span></span> <span data-ttu-id="15c66-483">Překlopit animace bude nyní provést a výsledek (*opravte nesprávné*) se zobrazí.</span><span class="sxs-lookup"><span data-stu-id="15c66-483">The flip animation will now take place and the result (*correct/incorrect*) will be displayed.</span></span>
17. <span data-ttu-id="15c66-484">(Volitelné) Přepněte **Git Bash** konzoly a spusťte následující příkazy se vrátit k předchozí potvrzení změn.</span><span class="sxs-lookup"><span data-stu-id="15c66-484">(Optional) Switch to the **Git Bash** console and execute the following commands to revert to the previous commit.</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-485">Tyto příkazy vytvoří nové potvrzení, který vrátí zpět všechny změny v úložišti Git, které byly provedeny v chybný potvrzení.</span><span class="sxs-lookup"><span data-stu-id="15c66-485">These commands create a new commit that undoes all changes in the Git repository that were made in the bad commit.</span></span> <span data-ttu-id="15c66-486">Azure pak znovu nasadit aplikaci pomocí nové potvrzení.</span><span class="sxs-lookup"><span data-stu-id="15c66-486">Azure will then redeploy the application using the new commit.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample16.cmd)]

<a id="Exercise4"></a>
### <a name="exercise-4-scaling-using-azure-storage"></a><span data-ttu-id="15c66-487">Cvičení 4: Škálování s využitím služby Azure Storage</span><span class="sxs-lookup"><span data-stu-id="15c66-487">Exercise 4: Scaling Using Azure Storage</span></span>

<span data-ttu-id="15c66-488">**Objekty BLOB** představují nejjednodušší způsob, jak ukládat velké objemy nestrukturovaných textových nebo binárních dat jako jsou videa, zvukové soubory a obrázky.</span><span class="sxs-lookup"><span data-stu-id="15c66-488">**Blobs** are the simplest way to store large amounts of unstructured text or binary data such as video, audio and images.</span></span> <span data-ttu-id="15c66-489">Přesunutí statický obsah aplikace do úložiště, umožňuje škálovat aplikaci podle poskytování obrázků nebo dokumentů přímo do prohlížeče.</span><span class="sxs-lookup"><span data-stu-id="15c66-489">Moving the static content of your application to Storage, helps to scale your application by serving images or documents directly to the browser.</span></span>

<span data-ttu-id="15c66-490">V tomto cvičení se přesune statický obsah aplikace do kontejneru objektů Blob.</span><span class="sxs-lookup"><span data-stu-id="15c66-490">In this exercise, you will move the static content of your application to a Blob container.</span></span> <span data-ttu-id="15c66-491">Pak se konfigurace aplikace pro přidání **přepisu adresy URL technologie ASP.NET pravidlo** v **Web.config** přesměrovat obsah do kontejneru objektů Blob.</span><span class="sxs-lookup"><span data-stu-id="15c66-491">Then you will configure your application to add an **ASP.NET URL rewrite rule** in the **Web.config** to redirect your content to the Blob container.</span></span>

<a id="Ex4Task1"></a>
#### <a name="task-1--creating-an-azure-storage-account"></a><span data-ttu-id="15c66-492">Úloha 1 – Vytvoření účtu služby Azure Storage</span><span class="sxs-lookup"><span data-stu-id="15c66-492">Task 1 – Creating an Azure Storage Account</span></span>

<span data-ttu-id="15c66-493">V této úloze se dozvíte, jak vytvořit nový účet úložiště pomocí portálu pro správu.</span><span class="sxs-lookup"><span data-stu-id="15c66-493">In this task you will learn how to create a new storage account using the management portal.</span></span>

1. <span data-ttu-id="15c66-494">Přejděte [portál pro správu Azure](https://manage.windowsazure.com) a přihlaste se pomocí účtu Microsoft, který je spojen s vaším předplatným.</span><span class="sxs-lookup"><span data-stu-id="15c66-494">Navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
2. <span data-ttu-id="15c66-495">Vyberte **nové | Data Services | Úložiště | Příkaz pro rychlé vytvoření** a začněte vytvářet nový účet úložiště.</span><span class="sxs-lookup"><span data-stu-id="15c66-495">Select **New | Data Services | Storage | Quick Create** to start creating a new storage account.</span></span> <span data-ttu-id="15c66-496">Zadejte jedinečný název pro účet a vyberte **oblasti** ze seznamu.</span><span class="sxs-lookup"><span data-stu-id="15c66-496">Enter a unique name for the account and select a **Region** from the list.</span></span> <span data-ttu-id="15c66-497">Klikněte na tlačítko **vytvořit účet úložiště** pokračujte.</span><span class="sxs-lookup"><span data-stu-id="15c66-497">Click **Create Storage Account** to continue.</span></span>

    <span data-ttu-id="15c66-498">![Vytváří se nový účet úložiště](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "vytváření nového účtu úložiště")</span><span class="sxs-lookup"><span data-stu-id="15c66-498">![Creating a new Storage Account](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "Creating a new Storage Account")</span></span>

    <span data-ttu-id="15c66-499">*Vytváří se nový účet úložiště*</span><span class="sxs-lookup"><span data-stu-id="15c66-499">*Creating a new storage account*</span></span>
3. <span data-ttu-id="15c66-500">V **úložiště** části, počkejte, dokud stav nového účtu úložiště se změní na *Online* budete pokračovat následujícím krokem.</span><span class="sxs-lookup"><span data-stu-id="15c66-500">In the **Storage** section, wait until the status of the new storage account changes to *Online* in order to continue with the following step.</span></span>

    <span data-ttu-id="15c66-501">![Účet úložiště vytvořený](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "vytvořený účet úložiště")</span><span class="sxs-lookup"><span data-stu-id="15c66-501">![Storage Account created](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "Storage Account created")</span></span>

    <span data-ttu-id="15c66-502">*Vytvoření účtu úložiště*</span><span class="sxs-lookup"><span data-stu-id="15c66-502">*Storage Account created*</span></span>
4. <span data-ttu-id="15c66-503">Klikněte na název účtu úložiště a pak klikněte na tlačítko **řídicí panel** odkazu v horní části stránky.</span><span class="sxs-lookup"><span data-stu-id="15c66-503">Click on the storage account name and then click the **Dashboard** link at the top of the page.</span></span> <span data-ttu-id="15c66-504">**Řídicí panel** stránce vám poskytne informace o stavu účtu a koncové body služby, které lze použít v rámci vašich aplikací.</span><span class="sxs-lookup"><span data-stu-id="15c66-504">The **Dashboard** page provides you with information about the status of the account and the service endpoints that can be used within your applications.</span></span>

    <span data-ttu-id="15c66-505">![Zobrazení řídicího panelu úložiště účtu](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "zobrazení řídicího panelu účtu úložiště")</span><span class="sxs-lookup"><span data-stu-id="15c66-505">![Displaying the Storage Account Dashboard](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "Displaying the Storage Account Dashboard")</span></span>

    <span data-ttu-id="15c66-506">*Zobrazení řídicí panel účtu úložiště*</span><span class="sxs-lookup"><span data-stu-id="15c66-506">*Displaying the Storage Account Dashboard*</span></span>
5. <span data-ttu-id="15c66-507">Klikněte na tlačítko **spravovat přístupové klíče** tlačítko v navigačním panelu.</span><span class="sxs-lookup"><span data-stu-id="15c66-507">Click the **Manage Access Keys** button in the navigation bar.</span></span>

    <span data-ttu-id="15c66-508">![Tlačítko Spravovat přístupové klíče](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "tlačítko Spravovat přístupové klíče")</span><span class="sxs-lookup"><span data-stu-id="15c66-508">![Manage Access Keys button](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "Manage Access Keys button")</span></span>

    <span data-ttu-id="15c66-509">*Správa přístupových klíčů tlačítko*</span><span class="sxs-lookup"><span data-stu-id="15c66-509">*Manage Access Keys button*</span></span>
6. <span data-ttu-id="15c66-510">V **spravovat přístupové klíče** dialogové okno, kopie **název účtu úložiště** a **primární přístupový klíč** , kdykoli budete potřebovat v následujícím cvičení.</span><span class="sxs-lookup"><span data-stu-id="15c66-510">In the **Manage Access Keys** dialog box, copy the **Storage Account Name** and **Primary Access Key** as you will need them in the following exercise.</span></span> <span data-ttu-id="15c66-511">Zavřete dialogové okno.</span><span class="sxs-lookup"><span data-stu-id="15c66-511">Then, close the dialog box.</span></span>

    <span data-ttu-id="15c66-512">![Přístupový klíč dialogového okna Spravovat](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "dialogové okno Spravovat přístupový klíč")</span><span class="sxs-lookup"><span data-stu-id="15c66-512">![Manage Access Key dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "Manage Access Key dialog box")</span></span>

    <span data-ttu-id="15c66-513">*Přístupový klíč dialogového okna Spravovat*</span><span class="sxs-lookup"><span data-stu-id="15c66-513">*Manage Access Key dialog box*</span></span>

<a id="Ex4Task2"></a>
#### <a name="task-2--uploading-an-asset-to-azure-blob-storage"></a><span data-ttu-id="15c66-514">Úloha 2 – nahrání prostředek do úložiště objektů Blob v Azure</span><span class="sxs-lookup"><span data-stu-id="15c66-514">Task 2 – Uploading an Asset to Azure Blob Storage</span></span>

<span data-ttu-id="15c66-515">V této úloze použijete okno Průzkumníka serveru ze sady Visual Studio pro připojení k účtu úložiště.</span><span class="sxs-lookup"><span data-stu-id="15c66-515">In this task, you will use the Server Explorer window from Visual Studio to connect to your storage account.</span></span> <span data-ttu-id="15c66-516">Pak vytvoříte kontejner objektů blob a odeslat soubor s logem kvíz nadšence do kontejneru.</span><span class="sxs-lookup"><span data-stu-id="15c66-516">You will then create a blob container and upload a file with the Geek Quiz logo to the container.</span></span>

1. <span data-ttu-id="15c66-517">Přepnout na instanci aplikace Visual Studio s **GeekQuiz** řešení v předchozím cvičení.</span><span class="sxs-lookup"><span data-stu-id="15c66-517">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="15c66-518">V panelu nabídky vyberte **zobrazení** a potom klikněte na tlačítko **Průzkumníka serveru**.</span><span class="sxs-lookup"><span data-stu-id="15c66-518">From the menu bar, select **View** and then click **Server Explorer**.</span></span>
3. <span data-ttu-id="15c66-519">V **Průzkumníka serveru**, klikněte pravým tlačítkem myši **Azure** uzel a vyberte možnost **připojit se k Azure...** . Přihlaste se pomocí účtu Microsoft, který je spojen s vaším předplatným.</span><span class="sxs-lookup"><span data-stu-id="15c66-519">In **Server Explorer**, right-click the **Azure** node and select **Connect to Azure...**. Sign in using the Microsoft account associated with your subscription.</span></span>

    ![Připojte se k Windows Azure](maintainable-azure-websites-managing-change-and-scale/_static/image62.png)

    <span data-ttu-id="15c66-521">*Připojení k Azure*</span><span class="sxs-lookup"><span data-stu-id="15c66-521">*Connect to Azure*</span></span>
4. <span data-ttu-id="15c66-522">Rozbalte **Azure** uzel, klikněte pravým tlačítkem na **úložiště** a vyberte **připojit externí úložiště...** .</span><span class="sxs-lookup"><span data-stu-id="15c66-522">Expand the **Azure** node, right-click **Storage** and select **Attach External Storage...**.</span></span>
5. <span data-ttu-id="15c66-523">V **přidat nový účet úložiště** dialogového okna zadejte **název účtu** a **klíč účtu** jste získali v předchozí úloze a klikněte na **OK**.</span><span class="sxs-lookup"><span data-stu-id="15c66-523">In the **Add New Storage Account** dialog box, enter the **Account name** and **Account key** you obtained in the previous task and click **OK**.</span></span>

    ![Přidat nový účet úložiště – dialogové okno](maintainable-azure-websites-managing-change-and-scale/_static/image63.png)

    <span data-ttu-id="15c66-525">*Přidat nový účet úložiště – dialogové okno*</span><span class="sxs-lookup"><span data-stu-id="15c66-525">*Add New Storage Account dialog box*</span></span>
6. <span data-ttu-id="15c66-526">Váš účet úložiště by se měla zobrazit v části **úložiště** uzlu.</span><span class="sxs-lookup"><span data-stu-id="15c66-526">Your storage account should appear under the **Storage** node.</span></span> <span data-ttu-id="15c66-527">Rozbalte účet úložiště, klikněte pravým tlačítkem na **objekty BLOB** a vyberte **vytvořit kontejner objektů Blob...** .</span><span class="sxs-lookup"><span data-stu-id="15c66-527">Expand your storage account, right-click **Blobs** and select **Create Blob Container...**.</span></span>

    <span data-ttu-id="15c66-528">![Vytvořit kontejner objektů Blob](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "vytvořit kontejner objektů Blob")</span><span class="sxs-lookup"><span data-stu-id="15c66-528">![Create Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Create Blob Container")</span></span>

    <span data-ttu-id="15c66-529">*Vytvořit kontejner objektů Blob*</span><span class="sxs-lookup"><span data-stu-id="15c66-529">*Create Blob Container*</span></span>
7. <span data-ttu-id="15c66-530">V **vytvořit kontejner objektů Blob** dialogového okna zadejte název kontejneru objektů blob a klikněte na tlačítko **OK**.</span><span class="sxs-lookup"><span data-stu-id="15c66-530">In the **Create Blob Container** dialog box, enter a name for the blob container and click **OK**.</span></span>

    <span data-ttu-id="15c66-531">![Dialogové okno vytvořit kontejner objektů Blob](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "dialogové okno vytvořit kontejner objektů Blob")</span><span class="sxs-lookup"><span data-stu-id="15c66-531">![Create Blob Container dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Create Blob Container dialog box")</span></span>

    <span data-ttu-id="15c66-532">*Vytvoření dialogového okna kontejner objektů Blob*</span><span class="sxs-lookup"><span data-stu-id="15c66-532">*Create Blob Container dialog box*</span></span>
8. <span data-ttu-id="15c66-533">Nový kontejner objektů blob měla být přidána do **objekty BLOB** uzlu.</span><span class="sxs-lookup"><span data-stu-id="15c66-533">The new blob container should be added to the **Blobs** node.</span></span> <span data-ttu-id="15c66-534">Změňte přístupová oprávnění v kontejneru zveřejněte kontejneru.</span><span class="sxs-lookup"><span data-stu-id="15c66-534">Change the access permissions in the container to make the container public.</span></span> <span data-ttu-id="15c66-535">Chcete-li to provést, klikněte pravým tlačítkem **image** kontejner a vyberte **vlastnosti**.</span><span class="sxs-lookup"><span data-stu-id="15c66-535">To do this, right-click the **images** container and select **Properties**.</span></span>

    <span data-ttu-id="15c66-536">![Image kontejneru vlastnosti](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "Image vlastnosti kontejneru")</span><span class="sxs-lookup"><span data-stu-id="15c66-536">![images container properties](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "images container properties")</span></span>

    <span data-ttu-id="15c66-537">*Vlastnosti kontejneru obrázků*</span><span class="sxs-lookup"><span data-stu-id="15c66-537">*Images container properties*</span></span>
9. <span data-ttu-id="15c66-538">V **vlastnosti** okno, nastaveno **veřejné oprávnění ke čtení** k **kontejneru**.</span><span class="sxs-lookup"><span data-stu-id="15c66-538">In the **Properties** window, set the **Public Read Access** to **Container**.</span></span>

    <span data-ttu-id="15c66-539">![Změna vlastnosti veřejné oprávnění ke čtení](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "při změně hodnoty vlastnosti veřejné oprávnění ke čtení")</span><span class="sxs-lookup"><span data-stu-id="15c66-539">![Changing public read access property](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "Changing public read access property")</span></span>

    <span data-ttu-id="15c66-540">*Změna vlastnosti veřejné oprávnění ke čtení*</span><span class="sxs-lookup"><span data-stu-id="15c66-540">*Changing public read access property*</span></span>
10. <span data-ttu-id="15c66-541">Po zobrazení výzvy, pokud jste si jistí, kterou chcete změnit vlastnost veřejného přístupu, klikněte na tlačítko **Ano**.</span><span class="sxs-lookup"><span data-stu-id="15c66-541">When prompted if you are sure you want to change the public access property, click **Yes**.</span></span>

    <span data-ttu-id="15c66-542">![Microsoft Visual Studio upozornění](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "upozornění Microsoft Visual Studio")</span><span class="sxs-lookup"><span data-stu-id="15c66-542">![Microsoft Visual Studio warning](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio warning")</span></span>

    <span data-ttu-id="15c66-543">*Microsoft Visual Studio upozornění*</span><span class="sxs-lookup"><span data-stu-id="15c66-543">*Microsoft Visual Studio warning*</span></span>
11. <span data-ttu-id="15c66-544">V **Průzkumníka serveru**, klikněte pravým tlačítkem **image** kontejner objektů blob a vyberte **kontejner objektů Blob zobrazení**.</span><span class="sxs-lookup"><span data-stu-id="15c66-544">In **Server Explorer**, right-click in the **images** blob container and select **View Blob Container**.</span></span>

    <span data-ttu-id="15c66-545">![Zobrazit kontejner objektů Blob](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "zobrazení kontejner objektů Blob")</span><span class="sxs-lookup"><span data-stu-id="15c66-545">![View Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "View Blob Container")</span></span>

    <span data-ttu-id="15c66-546">*Zobrazit kontejner objektů Blob*</span><span class="sxs-lookup"><span data-stu-id="15c66-546">*View Blob Container*</span></span>
12. <span data-ttu-id="15c66-547">Kontejner imagí by se otevřít v novém okně a legenda s žádné položky, které se má zobrazit.</span><span class="sxs-lookup"><span data-stu-id="15c66-547">The images container should open in a new window and a legend with no entries should be shown.</span></span> <span data-ttu-id="15c66-548">Klikněte na tlačítko **nahrát** ikonu pro nahrání souboru do kontejneru objektů blob.</span><span class="sxs-lookup"><span data-stu-id="15c66-548">Click the **upload** icon to upload a file to the blob container.</span></span>

    <span data-ttu-id="15c66-549">![Image kontejneru s žádné položky](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Image kontejneru s žádné položky")</span><span class="sxs-lookup"><span data-stu-id="15c66-549">![Images container with no entries](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Images container with no entries")</span></span>

    <span data-ttu-id="15c66-550">*Image kontejneru s žádné položky*</span><span class="sxs-lookup"><span data-stu-id="15c66-550">*Images container with no entries*</span></span>
13. <span data-ttu-id="15c66-551">V **nahrát objekt Blob** dialogové okno, přejděte na **prostředky** složky testovacího prostředí.</span><span class="sxs-lookup"><span data-stu-id="15c66-551">In the **Upload Blob** dialog box, navigate to the **Assets** folder of the lab.</span></span> <span data-ttu-id="15c66-552">Vyberte **logo big.png** souboru a klikněte na tlačítko **otevřít**.</span><span class="sxs-lookup"><span data-stu-id="15c66-552">Select the **logo-big.png** file and click **Open**.</span></span>
14. <span data-ttu-id="15c66-553">Počkejte, až se soubor nahraje.</span><span class="sxs-lookup"><span data-stu-id="15c66-553">Wait until the file is uploaded.</span></span> <span data-ttu-id="15c66-554">Po dokončení nahrávání souboru by měl nacházet na Image kontejneru.</span><span class="sxs-lookup"><span data-stu-id="15c66-554">When the upload completes, the file should be listed in the images container.</span></span> <span data-ttu-id="15c66-555">Klikněte pravým tlačítkem na položku soubor a vyberte **kopírování adresy URL**.</span><span class="sxs-lookup"><span data-stu-id="15c66-555">Right-click the file entry and select **Copy URL**.</span></span>

    <span data-ttu-id="15c66-556">![Zkopírujte adresu URL objektu blob](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "zkopírujte adresu URL souboru objektu blob")</span><span class="sxs-lookup"><span data-stu-id="15c66-556">![Copy blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "Copy blob file URL")</span></span>

    <span data-ttu-id="15c66-557">*Zkopírujte adresu URL objektu blob*</span><span class="sxs-lookup"><span data-stu-id="15c66-557">*Copy blob URL*</span></span>
15. <span data-ttu-id="15c66-558">Spusťte aplikaci Internet Explorer a vložte adresu URL.</span><span class="sxs-lookup"><span data-stu-id="15c66-558">Open Internet Explorer and paste the URL.</span></span> <span data-ttu-id="15c66-559">Na následujícím obrázku by měla být zobrazená v prohlížeči.</span><span class="sxs-lookup"><span data-stu-id="15c66-559">The following image should be shown in the browser.</span></span>

    <span data-ttu-id="15c66-560">![Obrázek loga big.png z úložiště objektů Blob Windows](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo big.png image ze služby storage")</span><span class="sxs-lookup"><span data-stu-id="15c66-560">![logo-big.png image from Windows Blob Storage](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo-big.png image from storage")</span></span>

    <span data-ttu-id="15c66-561">*Logo big.png image z Azure Blob Storage*</span><span class="sxs-lookup"><span data-stu-id="15c66-561">*logo-big.png image from Azure Blob Storage*</span></span>

<a id="Ex4Task3"></a>
#### <a name="task-3--updating-the-solution-to-consume-static-content-from-azure-blob-storage"></a><span data-ttu-id="15c66-562">Úloha 3 – aktualizuje se řešení využívají statický obsah z úložiště objektů Blob v Azure</span><span class="sxs-lookup"><span data-stu-id="15c66-562">Task 3 – Updating the Solution to Consume Static Content from Azure Blob Storage</span></span>

<span data-ttu-id="15c66-563">V této úloze nakonfigurujete **GeekQuiz** řešení, které využívají obrázek nahraný do Azure Blob Storage (a nikoli obraz umístěný ve webové aplikaci) tak, že přidáte pravidlo pro přepis adres URL technologie ASP.NET, v **web.config**souboru.</span><span class="sxs-lookup"><span data-stu-id="15c66-563">In this task, you will configure the **GeekQuiz** solution to consume the image uploaded to Azure Blob Storage (instead of the image located in the web app) by adding an ASP.NET URL rewrite rule in the **web.config** file.</span></span>

1. <span data-ttu-id="15c66-564">V sadě Visual Studio, otevřete **Web.config** soubor uvnitř **GeekQuiz** projektu a vyhledejte **&lt;system.webServer&gt;** elementu.</span><span class="sxs-lookup"><span data-stu-id="15c66-564">In Visual Studio, open the **Web.config** file inside the **GeekQuiz** project and locate the **&lt;system.webServer&gt;** element.</span></span>
2. <span data-ttu-id="15c66-565">Přidejte následující kód pro přidání přepisování adres URL pravidla aktualizace zástupného textu s názvem vašeho účtu úložiště.</span><span class="sxs-lookup"><span data-stu-id="15c66-565">Add the following code to add an URL rewrite rule, updating the placeholder with your storage account name.</span></span>

    <span data-ttu-id="15c66-566">(Fragment - kódu *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span><span class="sxs-lookup"><span data-stu-id="15c66-566">(Code Snippet - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span></span>

    [!code-xml[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample17.xml)]

    > [!NOTE]
    > <span data-ttu-id="15c66-567">Přepisování adres URL je proces zachycení příchozí webové žádosti a požadavku přesměrování na jiný prostředek.</span><span class="sxs-lookup"><span data-stu-id="15c66-567">URL rewriting is the process of intercepting an incoming Web request and redirecting the request to a different resource.</span></span> <span data-ttu-id="15c66-568">Adresa URL přepsání pravidel říká přepis adres modul, když se požadavek musí přesměrovat a kde by měl být přesměrován.</span><span class="sxs-lookup"><span data-stu-id="15c66-568">The URL rewriting rules tells the rewriting engine when a request needs to be redirected, and where should they be redirected.</span></span> <span data-ttu-id="15c66-569">Pravidlo pro přepis adres se skládá ze dvou řetězců: model v požadované adrese URL (obvykle, pomocí regulárních výrazů), a řetězec pro nahrazení vzoru, pokud nalezen.</span><span class="sxs-lookup"><span data-stu-id="15c66-569">A rewriting rule is composed of two strings: the pattern to look for in the requested URL (usually, using regular expressions), and the string to replace the pattern with, if found.</span></span> <span data-ttu-id="15c66-570">Další informace najdete v tématu [přepisování adres URL v technologii ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span><span class="sxs-lookup"><span data-stu-id="15c66-570">For more information, see [URL Rewriting in ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span></span>
3. <span data-ttu-id="15c66-571">Stisknutím klávesy **stisknutím CTRL + S** a uložte změny.</span><span class="sxs-lookup"><span data-stu-id="15c66-571">Press **CTRL + S** to save the changes.</span></span>
4. <span data-ttu-id="15c66-572">Otevřete novou **Git Bash** konzoly nasaďte aktualizovanou aplikaci do služby Azure App Service.</span><span class="sxs-lookup"><span data-stu-id="15c66-572">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
5. <span data-ttu-id="15c66-573">Spusťte následující příkazy, které odešlete změny do Azure.</span><span class="sxs-lookup"><span data-stu-id="15c66-573">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="15c66-574">Aktualizace *[YOUR cesta aplikace]* zástupný symbol s cestou **GeekQuiz** řešení.</span><span class="sxs-lookup"><span data-stu-id="15c66-574">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="15c66-575">Zobrazí se výzva k zadání hesla nasazení.</span><span class="sxs-lookup"><span data-stu-id="15c66-575">You will be prompted for your deployment password.</span></span>
    ...
    <span data-ttu-id="15c66-577">*Nasazení aktualizací do Azure*</span><span class="sxs-lookup"><span data-stu-id="15c66-577">*Deploying update to Azure*</span></span>

<a id="Ex4Task4"></a>
#### <a name="task-4--verification"></a><span data-ttu-id="15c66-578">Úloha 4 – ověření</span><span class="sxs-lookup"><span data-stu-id="15c66-578">Task 4 – Verification</span></span>

<span data-ttu-id="15c66-579">V této úloze budete používat **aplikace Internet Explorer** a přejděte **Informatik kvíz** aplikace a zkontrolujte, že adresa URL přepsat pravidlo pro Image funguje a můžete se přesměrují na image hostovanou v **objektů Blob v Azure Úložiště**.</span><span class="sxs-lookup"><span data-stu-id="15c66-579">In this task you will use **Internet Explorer** to browse the **Geek Quiz** application and check that the URL rewrite rule for images works and you are redirected to the image hosted on **Azure Blob Storage**.</span></span>

1. <span data-ttu-id="15c66-580">Otevřete aplikaci Internet Explorer a přejděte do své webové aplikace (například `http://<your-web-site>.azurewebsites.net`).</span><span class="sxs-lookup"><span data-stu-id="15c66-580">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="15c66-581">Přihlaste se pomocí dříve vytvořeného přihlašovacích údajů.</span><span class="sxs-lookup"><span data-stu-id="15c66-581">Log in using the previously created credentials.</span></span>

    <span data-ttu-id="15c66-582">![Zobrazuje webovou aplikaci Informatik kvíz s imagí](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "zobrazující webovou aplikaci Informatik kvíz s imagí")</span><span class="sxs-lookup"><span data-stu-id="15c66-582">![Showing the Geek Quiz web app with the image](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "Showing the Geek Quiz web app with the image")</span></span>

    <span data-ttu-id="15c66-583">*Zobrazuje webovou aplikaci Informatik kvíz s imagí*</span><span class="sxs-lookup"><span data-stu-id="15c66-583">*Showing the Geek Quiz web app with the image*</span></span>
2. <span data-ttu-id="15c66-584">Stisknutím klávesy **F12** ke spuštění nástroje pro vývoj, vyberte **sítě** kartu a spusťte záznam.</span><span class="sxs-lookup"><span data-stu-id="15c66-584">Press **F12** to launch the development tools, select the **Network** tab and start recording.</span></span>

    <span data-ttu-id="15c66-585">![Spouštění nahrávání sítě](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "od sítě záznam")</span><span class="sxs-lookup"><span data-stu-id="15c66-585">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "Starting network recording")</span></span>

    <span data-ttu-id="15c66-586">*Spouští se záznam sítě*</span><span class="sxs-lookup"><span data-stu-id="15c66-586">*Starting network recording*</span></span>
3. <span data-ttu-id="15c66-587">Stisknutím klávesy **CTRL + F5** aktualizujte webovou stránku.</span><span class="sxs-lookup"><span data-stu-id="15c66-587">Press **CTRL + F5** to refresh the web page.</span></span>
4. <span data-ttu-id="15c66-588">Po dokončení načítání stránky byste měli vidět požadavku HTTP pro **/img/logo-big.png** adresu URL pomocí protokolu HTTP **301** výsledek (přesměrování) a další žádost o `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` adresu URL s protokolem HTTP **200** výsledek.</span><span class="sxs-lookup"><span data-stu-id="15c66-588">Once the page has finished loading, you should see an HTTP request for the **/img/logo-big.png** URL with an HTTP **301** result (redirect) and another request for `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL with a HTTP **200** result.</span></span>

    <span data-ttu-id="15c66-589">![Ověřuje se adresa URL přesměrování](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "zobrazující přesměrování v nástroje pro vývojáře")</span><span class="sxs-lookup"><span data-stu-id="15c66-589">![Verifying the URL redirect](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "Showing the redirect in Dev Tools")</span></span>

    <span data-ttu-id="15c66-590">*Ověřuje se adresa URL pro přesměrování*</span><span class="sxs-lookup"><span data-stu-id="15c66-590">*Verifying the URL redirect*</span></span>

<a id="Exercise5"></a>
### <a name="exercise-5-using-autoscale-for-web-apps"></a><span data-ttu-id="15c66-591">Cvičení 5: Použití automatického škálování pro webové aplikace</span><span class="sxs-lookup"><span data-stu-id="15c66-591">Exercise 5: Using Autoscale for Web Apps</span></span>

> [!NOTE]
> <span data-ttu-id="15c66-592">Tento postup je volitelné, protože vyžaduje podporu pro zatížení webové &amp; testování výkonu, která je dostupná jenom pro **Visual Studio 2013 Ultimate Edition**.</span><span class="sxs-lookup"><span data-stu-id="15c66-592">This exercise is optional, since it requires support for Web Load &amp; Performance Testing which is only available for **Visual Studio 2013 Ultimate Edition**.</span></span> <span data-ttu-id="15c66-593">Další informace o konkrétní součásti, které Visual Studio 2013, porovnat verze [tady](https://www.microsoft.com/visualstudio/eng/products/compare).</span><span class="sxs-lookup"><span data-stu-id="15c66-593">For more information on specific Visual Studio 2013 features, compare versions [here](https://www.microsoft.com/visualstudio/eng/products/compare).</span></span>


<span data-ttu-id="15c66-594">**Azure App Service Web Apps** poskytuje funkci automatického škálování pro web apps spouštěná ve **standardní režim**.</span><span class="sxs-lookup"><span data-stu-id="15c66-594">**Azure App Service Web Apps** provides the Autoscale feature for web apps running in **Standard Mode**.</span></span> <span data-ttu-id="15c66-595">Automatické škálování umožňuje Azure se dá automaticky škálovat počet instancí webové aplikace v závislosti na zatížení.</span><span class="sxs-lookup"><span data-stu-id="15c66-595">Autoscale lets Azure automatically scale the instance count of your web app depending on the load.</span></span> <span data-ttu-id="15c66-596">Když je povolené automatické škálování, Azure kontroluje každých pět minut procesoru vaší webové aplikace a přidá instancí podle potřeby v daném okamžiku v čase.</span><span class="sxs-lookup"><span data-stu-id="15c66-596">When Autoscale is enabled, Azure checks the CPU of your web app once every five minutes and adds instances as needed at that point in time.</span></span> <span data-ttu-id="15c66-597">Pokud bude malé využití procesoru, Azure odebere instance každé dvě hodiny zajistit, že není snížený výkon webové aplikace.</span><span class="sxs-lookup"><span data-stu-id="15c66-597">If the CPU usage is low, Azure will remove instances once every two hours to ensure that the performance of your web app is not degraded.</span></span>

<span data-ttu-id="15c66-598">V tomto cvičení jste projít kroky potřebnými ke konfiguraci **automatického škálování** funkce pro **kvíz Informatik** webové aplikace.</span><span class="sxs-lookup"><span data-stu-id="15c66-598">In this exercise you will go through the steps required to configure the **Autoscale** feature for the **Geek Quiz** web app.</span></span> <span data-ttu-id="15c66-599">Tato funkce bude ověřte spuštěním zátěžového testu sady Visual Studio pro generování dostatečného zatížení procesoru v aplikaci k aktivaci instance upgrade.</span><span class="sxs-lookup"><span data-stu-id="15c66-599">You will verify this feature by running a Visual Studio load test to generate enough CPU load on the application to trigger an instance upgrade.</span></span>

<a id="Ex5Task1"></a>
#### <a name="task-1--configuring-autoscale-based-on-the-cpu-metric"></a><span data-ttu-id="15c66-600">Úloha 1 – konfigurace automatického škálování na základě metriky využití procesoru</span><span class="sxs-lookup"><span data-stu-id="15c66-600">Task 1 – Configuring Autoscale Based on the CPU Metric</span></span>

<span data-ttu-id="15c66-601">V této úloze bude používat portál pro správu Azure k povolení této funkce automatického škálování pro webovou aplikaci, kterou jste vytvořili v cvičení 2.</span><span class="sxs-lookup"><span data-stu-id="15c66-601">In this task you will use the Azure management portal to enable the Autoscale feature for the web app you created in Exercise 2.</span></span>

1. <span data-ttu-id="15c66-602">V [portál pro správu Azure](https://manage.windowsazure.com/)vyberte **Websites** a klikněte na webovou aplikaci, kterou jste vytvořili v cvičení 2.</span><span class="sxs-lookup"><span data-stu-id="15c66-602">In the [Azure management portal](https://manage.windowsazure.com/), select **Websites** and click the web app you created in Exercise 2.</span></span>
2. <span data-ttu-id="15c66-603">Přejděte **škálování** stránky.</span><span class="sxs-lookup"><span data-stu-id="15c66-603">Navigate to the **Scale** page.</span></span> <span data-ttu-id="15c66-604">V části **kapacity** vyberte **procesoru** pro **škálování podle metriky** konfigurace.</span><span class="sxs-lookup"><span data-stu-id="15c66-604">Under the **capacity** section, select **CPU** for the **Scale by Metric** configuration.</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-605">Při horizontálním škálování podle využití procesoru, Azure dynamicky přizpůsobí počet instancí, které aplikace používá, pokud se změní využití procesoru.</span><span class="sxs-lookup"><span data-stu-id="15c66-605">When scaling by CPU, Azure dynamically adjusts the number of instances that the app uses if the CPU usage changes.</span></span>

    <span data-ttu-id="15c66-606">![Výběr škálování podle využití procesoru](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "výběrem procesoru metriky pro automatické škálování")</span><span class="sxs-lookup"><span data-stu-id="15c66-606">![Selecting to scale by CPU](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "Selecting the CPU metric for auto scaling")</span></span>

    <span data-ttu-id="15c66-607">*Výběr škálování podle využití procesoru*</span><span class="sxs-lookup"><span data-stu-id="15c66-607">*Selecting to scale by CPU*</span></span>
3. <span data-ttu-id="15c66-608">Změnit **cílový procesor** konfiguraci **20**-**40** procent.</span><span class="sxs-lookup"><span data-stu-id="15c66-608">Change the **Target CPU** configuration to **20**-**40** percent.</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-609">Tento rozsah představuje průměrné využití procesoru pro vaši webovou aplikaci.</span><span class="sxs-lookup"><span data-stu-id="15c66-609">This range represents the average CPU usage for your web app.</span></span> <span data-ttu-id="15c66-610">Azure bude přidávat nebo odebírat instance, které chcete zachovat svou webovou aplikaci v tomto rozsahu.</span><span class="sxs-lookup"><span data-stu-id="15c66-610">Azure will add or remove instances to keep your web app in this range.</span></span> <span data-ttu-id="15c66-611">Zadat minimální a maximální počet instancí používaných pro škálování v **počet instancí** konfigurace.</span><span class="sxs-lookup"><span data-stu-id="15c66-611">The minimum and maximum number of instances used for scaling is specified in the **Instance Count** configuration.</span></span> <span data-ttu-id="15c66-612">Azure se nikdy dostanou nad nebo nad rámec tohoto limitu.</span><span class="sxs-lookup"><span data-stu-id="15c66-612">Azure will never go above or beyond that limit.</span></span>
    >
    > <span data-ttu-id="15c66-613">Výchozí hodnota **cílový procesor** hodnoty jsou změněny jen pro účely tohoto testovacího prostředí.</span><span class="sxs-lookup"><span data-stu-id="15c66-613">The default **Target CPU** values are modified just for the purposes of this lab.</span></span> <span data-ttu-id="15c66-614">Nakonfigurováním rozsahu procesoru malé hodnotami jsou zvyšuje šance na aktivační událost automatického škálování při moderování zatížení je umístěn na aplikaci.</span><span class="sxs-lookup"><span data-stu-id="15c66-614">By configuring the CPU range with small values, you are increasing the chances to trigger Autoscale when a moderate load is placed on the application.</span></span>

    <span data-ttu-id="15c66-615">![Změna cílového procesoru chcete být 20 až 40 procent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "změna cíl CPU, který má být 20 až 40 procent")</span><span class="sxs-lookup"><span data-stu-id="15c66-615">![Changing the target CPU to be between 20 and 40 percent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "Changing the target CPU to be between 20 and 40 percent")</span></span>

    <span data-ttu-id="15c66-616">*Změna cílový procesor bude 20 až 40 procent*</span><span class="sxs-lookup"><span data-stu-id="15c66-616">*Changing the Target CPU to be between 20 and 40 percent*</span></span>
4. <span data-ttu-id="15c66-617">Klikněte na tlačítko **Uložit** na panelu příkazů a uložte změny.</span><span class="sxs-lookup"><span data-stu-id="15c66-617">Click **Save** in the command bar to save the changes.</span></span>

<a id="Ex5Task2"></a>
#### <a name="task-2--load-testing-with-visual-studio"></a><span data-ttu-id="15c66-618">Úloha 2 – zátěžové testování pomocí sady Visual Studio</span><span class="sxs-lookup"><span data-stu-id="15c66-618">Task 2 – Load Testing with Visual Studio</span></span>

<span data-ttu-id="15c66-619">Teď, když **automatického škálování** byl nakonfigurován, musíte vytvořit **načíst projekt testu výkonu webu a** v sadě Visual Studio k vygenerování nějaké zatížení CPU na webovou aplikaci.</span><span class="sxs-lookup"><span data-stu-id="15c66-619">Now that **Autoscale** has been configured, you will create a **Web Performance and Load Test Project** in Visual Studio to generate some CPU load on your web app.</span></span>

1. <span data-ttu-id="15c66-620">Otevřít **Visual Studio Ultimate 2013** a vyberte **soubor | Nové | Projekt...**  spusťte nové řešení.</span><span class="sxs-lookup"><span data-stu-id="15c66-620">Open **Visual Studio Ultimate 2013** and select **File | New | Project...** to start a new solution.</span></span>

    <span data-ttu-id="15c66-621">![Vytvoření nového projektu](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "vytvoření nového projektu")</span><span class="sxs-lookup"><span data-stu-id="15c66-621">![Creating a new project](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "Creating a new project")</span></span>

    <span data-ttu-id="15c66-622">*Vytvoření nového projektu*</span><span class="sxs-lookup"><span data-stu-id="15c66-622">*Creating a new project*</span></span>
2. <span data-ttu-id="15c66-623">V **nový projekt** dialogu **webový výkon a projekt zátěžového testu** pod **Visual C# | Test** kartu. Ujistěte se, že **rozhraní .NET Framework 4.5** je vybrána, pojmenujte projekt *WebAndLoadTestProject*, zvolte **umístění** a klikněte na tlačítko **OK**.</span><span class="sxs-lookup"><span data-stu-id="15c66-623">In the **New Project** dialog box, select **Web Performance and Load Test Project** under the **Visual C# | Test** tab. Make sure **.NET Framework 4.5** is selected, name the project *WebAndLoadTestProject*, choose a **Location** and click **OK**.</span></span>

    <span data-ttu-id="15c66-624">![Vytvoření nového projektu webového a zátěžového testu](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "vytvoření nového projektu webového a zátěžového testu")</span><span class="sxs-lookup"><span data-stu-id="15c66-624">![Creating a new Web and Load Test project](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Creating a new Web and Load Test project")</span></span>

    <span data-ttu-id="15c66-625">*Vytvoření nového projektu webového a zátěžového testu*</span><span class="sxs-lookup"><span data-stu-id="15c66-625">*Creating a new Web and Load Test project*</span></span>
3. <span data-ttu-id="15c66-626">V **WebTest1.webtest** klikněte pravým tlačítkem na **WebTest1** uzel a klikněte na tlačítko **přidejte žádosti**.</span><span class="sxs-lookup"><span data-stu-id="15c66-626">In the **WebTest1.webtest** Right-click the **WebTest1** node and click **Add Request**.</span></span>

    <span data-ttu-id="15c66-627">![Žádost o přidání do WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "přidáním požadavku do WebTest1")</span><span class="sxs-lookup"><span data-stu-id="15c66-627">![Adding a request to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "Adding a request to WebTest1")</span></span>

    <span data-ttu-id="15c66-628">*Žádost o přidání do WebTest1*</span><span class="sxs-lookup"><span data-stu-id="15c66-628">*Adding a request to WebTest1*</span></span>
4. <span data-ttu-id="15c66-629">V **vlastnosti** okno nového uzlu žádost o aktualizaci **Url** vlastnost tak, aby odkazoval na adresu URL webové aplikace (například *[ http://geek-quiz.azurewebsites.net/ ](http://geek-quiz.azurewebsites.net/)*).</span><span class="sxs-lookup"><span data-stu-id="15c66-629">In the **Properties** window of the new request node, update the **Url** property to point to the URL of your web app (e.g. *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span></span>

    <span data-ttu-id="15c66-630">![Změna vlastnosti adresy Url](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "při změně hodnoty vlastnosti adresy Url")</span><span class="sxs-lookup"><span data-stu-id="15c66-630">![Changing the Url property](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Changing the Url property")</span></span>

    <span data-ttu-id="15c66-631">*Změna vlastnosti adresy Url*</span><span class="sxs-lookup"><span data-stu-id="15c66-631">*Changing the Url property*</span></span>
5. <span data-ttu-id="15c66-632">V **WebTest1.webtest** okna, klikněte pravým tlačítkem na **WebTest1** a klikněte na tlačítko **přidejte smyčku...** .</span><span class="sxs-lookup"><span data-stu-id="15c66-632">In the **WebTest1.webtest** window, right-click **WebTest1** and click **Add Loop...**.</span></span>

    <span data-ttu-id="15c66-633">![Přidání smyčky do WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "přidání smyčky do WebTest1")</span><span class="sxs-lookup"><span data-stu-id="15c66-633">![Adding a loop to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "Adding a loop to WebTest1")</span></span>

    <span data-ttu-id="15c66-634">*Přidání smyčky do WebTest1*</span><span class="sxs-lookup"><span data-stu-id="15c66-634">*Adding a loop to WebTest1*</span></span>
6. <span data-ttu-id="15c66-635">V **přidat podmíněné pravidlo a položky do cyklu** dialogové okno, vyberte **smyčky For Loop** pravidla a upravit následující vlastnosti.</span><span class="sxs-lookup"><span data-stu-id="15c66-635">In the **Add Conditional Rule and Items to Loop** dialog box, select the **For Loop** rule and modify the following properties.</span></span>

   1. <span data-ttu-id="15c66-636">**Ukončovací hodnota:** 1000</span><span class="sxs-lookup"><span data-stu-id="15c66-636">**Terminating value:** 1000</span></span>
   2. <span data-ttu-id="15c66-637">**Název parametru kontextu:** iterátoru</span><span class="sxs-lookup"><span data-stu-id="15c66-637">**Context Parameter Name:** Iterator</span></span>
   3. <span data-ttu-id="15c66-638">**Přírůstková hodnota:** 1</span><span class="sxs-lookup"><span data-stu-id="15c66-638">**Increment Value:** 1</span></span>

      <span data-ttu-id="15c66-639">![Vyberte pravidlo smyčku For a aktualizace vlastností](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "vyberte pravidlo smyčku For a aktualizace vlastností")</span><span class="sxs-lookup"><span data-stu-id="15c66-639">![Selecting the For Loop rule and updating the properties](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "Selecting the For Loop rule and updating the properties")</span></span>

      <span data-ttu-id="15c66-640">*Vyberte pravidlo smyčku For a aktualizace vlastností*</span><span class="sxs-lookup"><span data-stu-id="15c66-640">*Selecting the For Loop rule and updating the properties*</span></span>
7. <span data-ttu-id="15c66-641">V části **položky ve smyčce** vyberte žádosti, které jste předtím vytvořili pro první a poslední položka smyčky.</span><span class="sxs-lookup"><span data-stu-id="15c66-641">Under the **Items in loop** section, select the request you created previously to be the first and last item for the loop.</span></span> <span data-ttu-id="15c66-642">Klikněte na tlačítko **OK** pokračujte.</span><span class="sxs-lookup"><span data-stu-id="15c66-642">Click **OK** to continue.</span></span>

    <span data-ttu-id="15c66-643">![Výběr položek první a poslední smyčky](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "výběr položek první a poslední smyčky")</span><span class="sxs-lookup"><span data-stu-id="15c66-643">![Selecting the first and last items for the loop](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "Selecting the first and last items for the loop")</span></span>

    <span data-ttu-id="15c66-644">*Výběr položek první a poslední smyčky*</span><span class="sxs-lookup"><span data-stu-id="15c66-644">*Selecting the first and last items for the loop*</span></span>
8. <span data-ttu-id="15c66-645">V **Průzkumníku řešení**, klikněte pravým tlačítkem myši **WebAndLoadTestProject** projektu, rozbalte položku **přidat** nabídky a vybereme **zátěžového testu...** .</span><span class="sxs-lookup"><span data-stu-id="15c66-645">In **Solution Explorer**, right-click the **WebAndLoadTestProject** project, expand the **Add** menu and select **Load Test...**.</span></span>

    <span data-ttu-id="15c66-646">![Zátěžový Test do projektu přidáte, WebAndLoadTestProject](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "zátěžový Test do projektu přidáte, WebAndLoadTestProject")</span><span class="sxs-lookup"><span data-stu-id="15c66-646">![Adding a Load Test to the WebAndLoadTestProject project](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "Adding a Load Test to the WebAndLoadTestProject project")</span></span>

    <span data-ttu-id="15c66-647">*Zátěžový Test do projektu přidáte, WebAndLoadTestProject*</span><span class="sxs-lookup"><span data-stu-id="15c66-647">*Adding a Load Test to the WebAndLoadTestProject project*</span></span>
9. <span data-ttu-id="15c66-648">V **Průvodce novým zátěžovým testem** dialogové okno, klikněte na tlačítko **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-648">In the **New Load Test Wizard** dialog box, click **Next**.</span></span>

    <span data-ttu-id="15c66-649">![Průvodce novým zátěžovým testem](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "Průvodce novým zátěžovým testem")</span><span class="sxs-lookup"><span data-stu-id="15c66-649">![New Load Test Wizard](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "New Load Test Wizard")</span></span>

    <span data-ttu-id="15c66-650">*Průvodce novým zátěžovým testem*</span><span class="sxs-lookup"><span data-stu-id="15c66-650">*New Load Test Wizard*</span></span>
10. <span data-ttu-id="15c66-651">V **scénář** stránce **nepoužívat čas přemýšlení** a klikněte na tlačítko **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-651">In the **Scenario** page, select **Do not use think times** and click **Next**.</span></span>

    <span data-ttu-id="15c66-652">![Výběr nepoužívat čas přemýšlení](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "výběr nepoužívat čas přemýšlení")</span><span class="sxs-lookup"><span data-stu-id="15c66-652">![Selecting not to use think times](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Selecting not to use think times")</span></span>

    <span data-ttu-id="15c66-653">*Výběr nepoužívat čas přemýšlení*</span><span class="sxs-lookup"><span data-stu-id="15c66-653">*Selecting not to use think times*</span></span>
11. <span data-ttu-id="15c66-654">V **vzor zatížení** stránky, ujistěte se, že **konstantní zatížení** je vybraná možnost.</span><span class="sxs-lookup"><span data-stu-id="15c66-654">In the **Load Pattern** page, make sure that the **Constant Load** option is selected.</span></span> <span data-ttu-id="15c66-655">Změnit **počet uživatelů** nastavení **250** uživatele a klikněte na tlačítko **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-655">Change the **User Count** setting to **250** users and click **Next**.</span></span>

    <span data-ttu-id="15c66-656">![Změna počtu uživatelů na 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "změnu počtu uživatelů na 250")</span><span class="sxs-lookup"><span data-stu-id="15c66-656">![Changing the user count to 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "Changing the user count to 250")</span></span>

    <span data-ttu-id="15c66-657">*Změna počtu uživatelů na 250*</span><span class="sxs-lookup"><span data-stu-id="15c66-657">*Changing the user count to 250*</span></span>
12. <span data-ttu-id="15c66-658">V **testovat Model kombinace** stránce **na základě pořadí sekvenčního testu** a klikněte na tlačítko **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-658">In the **Test Mix Model** page, select **Based on sequential test order** and click **Next**.</span></span>

    <span data-ttu-id="15c66-659">![Výběr modelu kombinace testů](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Výběr modelu kombinace testů")</span><span class="sxs-lookup"><span data-stu-id="15c66-659">![Selecting the test mix model](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Selecting the test mix model")</span></span>

    <span data-ttu-id="15c66-660">*Výběr modelu kombinace testů*</span><span class="sxs-lookup"><span data-stu-id="15c66-660">*Selecting the test mix model*</span></span>
13. <span data-ttu-id="15c66-661">V **Model kombinace testů** klikněte na **přidat...**  přidat test do kombinaci.</span><span class="sxs-lookup"><span data-stu-id="15c66-661">In the **Test Mix Model** page, click **Add...** to add a test to the mix.</span></span>

    <span data-ttu-id="15c66-662">![Přidání testu do poměru testů](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "přidání testu do poměru testů")</span><span class="sxs-lookup"><span data-stu-id="15c66-662">![Adding a test to the test mix](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "Adding a test to the test mix")</span></span>

    <span data-ttu-id="15c66-663">*Přidání testu do poměru testů*</span><span class="sxs-lookup"><span data-stu-id="15c66-663">*Adding a test to the test mix*</span></span>
14. <span data-ttu-id="15c66-664">V **přidat testy** dialogové okno, klikněte dvakrát na **WebTest1** přidat test **vybrané testy** seznamu.</span><span class="sxs-lookup"><span data-stu-id="15c66-664">In the **Add Tests** dialog box, double-click **WebTest1** to add the test to the **Selected tests** list.</span></span> <span data-ttu-id="15c66-665">Klikněte na tlačítko **OK** pokračujte.</span><span class="sxs-lookup"><span data-stu-id="15c66-665">Click **OK** to continue.</span></span>

    <span data-ttu-id="15c66-666">![Přidat WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "přidání WebTest1 testu")</span><span class="sxs-lookup"><span data-stu-id="15c66-666">![Adding the WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "Adding the WebTest1 test")</span></span>

    <span data-ttu-id="15c66-667">*Přidat WebTest1 test*</span><span class="sxs-lookup"><span data-stu-id="15c66-667">*Adding the WebTest1 test*</span></span>
15. <span data-ttu-id="15c66-668">Zpátky **poměru testů** klikněte na **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-668">Back in the **Test Mix** page, click **Next**.</span></span>

    <span data-ttu-id="15c66-669">![Dokončení stránce poměru testů](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "dokončení stránce poměru testů")</span><span class="sxs-lookup"><span data-stu-id="15c66-669">![Completing the Test Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Completing the Test Mix page")</span></span>

    <span data-ttu-id="15c66-670">*Dokončení stránce poměru testů*</span><span class="sxs-lookup"><span data-stu-id="15c66-670">*Completing the Test Mix page*</span></span>
16. <span data-ttu-id="15c66-671">V **kombinaci sítí** klikněte na **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-671">In the **Network Mix** page, click **Next**.</span></span>

    <span data-ttu-id="15c66-672">![Kliknete na další na stránce kombinaci sítí](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "kliknete na další na stránce poměr sítí")</span><span class="sxs-lookup"><span data-stu-id="15c66-672">![Clicking next in the Network Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "Clicking next in the Network Mix page")</span></span>

    <span data-ttu-id="15c66-673">*Kliknutím na další stránce poměr sítí*</span><span class="sxs-lookup"><span data-stu-id="15c66-673">*Clicking next in the Network Mix page*</span></span>
17. <span data-ttu-id="15c66-674">V **kombinace prohlížečů** stránce **Internet Explorer 10.0** jako typ prohlížeče a klikněte na **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-674">In the **Browser Mix** page, select **Internet Explorer 10.0** as the browser type and click **Next**.</span></span>

    <span data-ttu-id="15c66-675">![Výběr typu prohlížeče](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "vyberete typ prohlížeče")</span><span class="sxs-lookup"><span data-stu-id="15c66-675">![Selecting the browser type](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "Selecting the browser type")</span></span>

    <span data-ttu-id="15c66-676">*Vyberte typ prohlížeče*</span><span class="sxs-lookup"><span data-stu-id="15c66-676">*Selecting the browser type*</span></span>
18. <span data-ttu-id="15c66-677">V **sady čítačů** klikněte na **Další**.</span><span class="sxs-lookup"><span data-stu-id="15c66-677">In the **Counter Sets** page, click **Next**.</span></span>

    <span data-ttu-id="15c66-678">![Kliknutím na další stránce sady čítačů](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "kliknutím na další stránce sady čítačů")</span><span class="sxs-lookup"><span data-stu-id="15c66-678">![Clicking Next in the Counter Sets page](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "Clicking Next in the Counter Sets page")</span></span>

    <span data-ttu-id="15c66-679">*Kliknutím na další stránce sady čítačů*</span><span class="sxs-lookup"><span data-stu-id="15c66-679">*Clicking Next in the Counter Sets page*</span></span>
19. <span data-ttu-id="15c66-680">V **parametrů běhu** nastavte **trvání zátěžového testu** k **5 minut** a klikněte na tlačítko **Dokončit**.</span><span class="sxs-lookup"><span data-stu-id="15c66-680">In the **Run Settings** page, set the **Load test duration** to **5 minutes** and click **Finish**.</span></span>

    <span data-ttu-id="15c66-681">![Nastavení trvání zátěžového testu na 5 minut](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "nastavení trvání zátěžového testu na 5 minut")</span><span class="sxs-lookup"><span data-stu-id="15c66-681">![Setting the load test duration to 5 minutes](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "Setting the load test duration to 5 minutes")</span></span>

    <span data-ttu-id="15c66-682">*Nastavení trvání zátěžového testu na 5 minut*</span><span class="sxs-lookup"><span data-stu-id="15c66-682">*Setting the load test duration to 5 minutes*</span></span>
20. <span data-ttu-id="15c66-683">V **Průzkumníka řešení**, dvakrát klikněte **Local.settings** souboru prozkoumat nastavení testu.</span><span class="sxs-lookup"><span data-stu-id="15c66-683">In **Solution Explorer**, double-click the **Local.settings** file to explore the test settings.</span></span> <span data-ttu-id="15c66-684">Ve výchozím nastavení používá Visual Studio místního počítače ke spuštění testů.</span><span class="sxs-lookup"><span data-stu-id="15c66-684">By default, Visual Studio uses your local computer to run the tests.</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-685">Alternativně můžete nakonfigurovat testovacího projektu pro spuštění zátěžových testů v cloudu pomocí **testovací plány Azure**.</span><span class="sxs-lookup"><span data-stu-id="15c66-685">Alternatively, you can configure your test project to run the load tests in the cloud using **Azure Test Plans**.</span></span> <span data-ttu-id="15c66-686">Azure testovací plány poskytuje cloudové zátěžové testování služba, která simuluje zatížení realističtější, jak se vyhnout omezení místní prostředí jako kapacity procesoru, paměti a šířky pásma sítě.</span><span class="sxs-lookup"><span data-stu-id="15c66-686">Azure Test Plans provides a cloud-based load testing service that simulates a more realistic load, avoiding local environment constraints like CPU capacity, available memory, and network bandwidth.</span></span> <span data-ttu-id="15c66-687">Další informace o používání Azure testovací plány pro spuštění zátěžových testů, naleznete v tématu [scénáře testování zatížení](/azure/devops/test/load-test/overview?view=vsts).</span><span class="sxs-lookup"><span data-stu-id="15c66-687">For more information about using Azure Test Plans to run load tests, see [Load testing scenarios](/azure/devops/test/load-test/overview?view=vsts).</span></span>

    ![Nastavení testu](maintainable-azure-websites-managing-change-and-scale/_static/image98.png)

<a id="Ex5Task3"></a>
#### <a name="task-3--autoscale-verification"></a><span data-ttu-id="15c66-689">Úloha 3 – ověření automatického škálování</span><span class="sxs-lookup"><span data-stu-id="15c66-689">Task 3 – Autoscale Verification</span></span>

<span data-ttu-id="15c66-690">Nyní spusťte zátěžový test, který jste vytvořili v předchozí úloze a naleznete v tématu jak webové aplikace chová při zatížení.</span><span class="sxs-lookup"><span data-stu-id="15c66-690">You will now execute the load test you created in the previous task and see how your web app behaves under load.</span></span>

1. <span data-ttu-id="15c66-691">V **Průzkumníka řešení**, dvakrát klikněte na panel **LoadTest1.loadtest** otevřete zátěžový test.</span><span class="sxs-lookup"><span data-stu-id="15c66-691">In **Solution Explorer**, double-click **LoadTest1.loadtest** to open the load test.</span></span>

    <span data-ttu-id="15c66-692">![Otevírání LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "otevírání LoadTest1.loadtest")</span><span class="sxs-lookup"><span data-stu-id="15c66-692">![Opening LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "Opening LoadTest1.loadtest")</span></span>

    <span data-ttu-id="15c66-693">*Otevírání LoadTest1.loadtest*</span><span class="sxs-lookup"><span data-stu-id="15c66-693">*Opening LoadTest1.loadtest*</span></span>
2. <span data-ttu-id="15c66-694">V **LoadTest1.loadtest** okna, klikněte na první tlačítko na panelu nástrojů ke spuštění zátěžového testu.</span><span class="sxs-lookup"><span data-stu-id="15c66-694">In the **LoadTest1.loadtest** window, click the first button in the toolbox to run the load test.</span></span>

    <span data-ttu-id="15c66-695">![Spuštění zátěžového testu](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "spuštění zátěžového testu")</span><span class="sxs-lookup"><span data-stu-id="15c66-695">![Running the load test](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "Running the load test")</span></span>

    <span data-ttu-id="15c66-696">*Spuštění zátěžového testu*</span><span class="sxs-lookup"><span data-stu-id="15c66-696">*Running the load test*</span></span>
3. <span data-ttu-id="15c66-697">Počkejte na dokončení zátěžového testu.</span><span class="sxs-lookup"><span data-stu-id="15c66-697">Wait until the load test completes.</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-698">Zátěžový test simuluje několik uživatelů, kteří současně zasílat požadavky do webové aplikace.</span><span class="sxs-lookup"><span data-stu-id="15c66-698">The load test simulates multiple users that send requests to the web app simultaneously.</span></span> <span data-ttu-id="15c66-699">Když test běží, můžete sledovat dostupné čítače, které chcete zjistit všechny chyby, upozornění nebo jiné informace související se zátěžovým testem.</span><span class="sxs-lookup"><span data-stu-id="15c66-699">When the test is running, you can monitor the available counters to detect any errors, warnings or other information related to your load test run.</span></span>

    <span data-ttu-id="15c66-700">![Zátěžový test spuštěn](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "čekání, až po dokončení zátěžového testu")</span><span class="sxs-lookup"><span data-stu-id="15c66-700">![Load test running](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "Waiting until the load test completes")</span></span>

    <span data-ttu-id="15c66-701">*Spuštění zátěžového testu*</span><span class="sxs-lookup"><span data-stu-id="15c66-701">*Load test running*</span></span>
4. <span data-ttu-id="15c66-702">Po dokončení testu, vraťte se do portálu pro správu a přejděte **škálování** stránce vaší webové aplikace.</span><span class="sxs-lookup"><span data-stu-id="15c66-702">Once the test completes, go back to the management portal and navigate to the **Scale** page of your web app.</span></span> <span data-ttu-id="15c66-703">V části **kapacity** oddílu, měli byste vidět v grafu, že se automaticky nasadí novou instanci.</span><span class="sxs-lookup"><span data-stu-id="15c66-703">Under the **capacity** section, you should see in the graph that a new instance was automatically deployed.</span></span>

    ![Automaticky nasadit novou instanci](maintainable-azure-websites-managing-change-and-scale/_static/image102.png)

    <span data-ttu-id="15c66-705">*Automaticky nasadit novou instanci*</span><span class="sxs-lookup"><span data-stu-id="15c66-705">*New instance automatically deployed*</span></span>

    > [!NOTE]
    > <span data-ttu-id="15c66-706">Může trvat několik minut, než se změna projeví v grafu (stiskněte **CTRL + F5** pravidelně obnovíte stránku).</span><span class="sxs-lookup"><span data-stu-id="15c66-706">It may take several minutes for the changes to appear in the graph (press **CTRL + F5** periodically to refresh the page).</span></span> <span data-ttu-id="15c66-707">Pokud se nezobrazí žádné změny, zkuste následující:</span><span class="sxs-lookup"><span data-stu-id="15c66-707">If you do not see any changes, you can try the following:</span></span>
    >
    > - <span data-ttu-id="15c66-708">Prodloužit dobu trvání zátěžového testu (například na **10 minut**)</span><span class="sxs-lookup"><span data-stu-id="15c66-708">Increase the duration of the load test (e.g. to **10 minutes**)</span></span>
    > - <span data-ttu-id="15c66-709">Omezit maximální a minimální hodnoty **cílový procesor** rozsah v konfiguraci automatického škálování webové aplikace</span><span class="sxs-lookup"><span data-stu-id="15c66-709">Reduce the maximum and minimum values of the **Target CPU** range in the Autoscale configuration of your web app</span></span>
    > - <span data-ttu-id="15c66-710">Spusťte zátěžový test v cloudu s využitím **testovací plány Azure**.</span><span class="sxs-lookup"><span data-stu-id="15c66-710">Run the load test in the cloud with **Azure Test Plans**.</span></span> <span data-ttu-id="15c66-711">Další informace o [zde](/azure/devops/test/load-test/index?view=vsts)</span><span class="sxs-lookup"><span data-stu-id="15c66-711">More information [here](/azure/devops/test/load-test/index?view=vsts)</span></span>

* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="15c66-712">Souhrn</span><span class="sxs-lookup"><span data-stu-id="15c66-712">Summary</span></span>

<span data-ttu-id="15c66-713">V této praktické laboratoři jste zjistili, jak nastavit a nasadit aplikaci do produkční webové aplikace v Azure.</span><span class="sxs-lookup"><span data-stu-id="15c66-713">In this hands-on lab, you learned how to set up and deploy your application to production web apps in Azure.</span></span> <span data-ttu-id="15c66-714">Jste spustili pomocí detekce a aktualizaci vašich databází pomocí **migrace Entity Framework Code First**, pak navázat tak, že nasazování nových verzí vašeho webu pomocí **Git** a vrácení zpět k provádění nejnovější stabilní verze vaší lokality.</span><span class="sxs-lookup"><span data-stu-id="15c66-714">You started by detecting and updating your databases using **Entity Framework Code First Migrations**, then continued by deploying new versions of your site using **Git** and performing rollbacks to the latest stable version of your site.</span></span> <span data-ttu-id="15c66-715">Kromě toho jste zjistili, jak škálovat aplikace pomocí úložiště přesunout statického obsahu do kontejneru objektů Blob.</span><span class="sxs-lookup"><span data-stu-id="15c66-715">Additionally, you learned how to scale your app using Storage to move your static content to a Blob container.</span></span>
