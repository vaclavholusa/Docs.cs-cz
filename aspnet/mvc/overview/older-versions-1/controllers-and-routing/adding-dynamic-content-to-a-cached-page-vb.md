...
Za normálních okolností při výstupu do mezipaměti na stránce pomocí &lt;OutputCache&gt; atribut, stránky se uloží do mezipaměti na serveru a klienta (webový prohlížeč). Při použití mezipaměti po nahrazení stránky se uloží do mezipaměti pouze na serveru.
...
Metoda RenderNews je předaný metodě WriteSubstitution(). Všimněte si, že RenderNews metoda není volána. Místo toho odkaz na metodu je předán WriteSubstitution() pomocí operátoru AddressOf.

Zobrazení Index se uloží do mezipaměti. Zobrazení je vrácený řadič v výpis 3. Všimněte si, že akce Index() opatřen s &lt;OutputCache&gt; atribut, který způsobí, že zobrazení indexu ukládat do mezipaměti po dobu 60 sekund.
...
Výpis 4 obsahuje modul jazyka Visual Basic, který zveřejňuje dvě metody: RenderBanner() a RenderBannerInternal(). Metoda RenderBanner() představuje skutečný pomocnou metodu. Tato metoda rozšiřuje standardní třída ASP.NET MVC HtmlHelper, tak, aby Html.RenderBanner() můžete volat v zobrazení stejně jako další metodu helper.
