...
Povolíte ukládání výstupu do mezipaměti přidáním atribut [OutputCache] akce jednotlivých řadiče nebo třídu celý kontroler. Například řadič v výpis 1 zpřístupní akci s názvem Index(). Výstup Index() akce se uloží do mezipaměti 10 sekund.
...
Ve výchozím nastavení, když pomocí atributu [OutputCache], obsah se uloží do mezipaměti ve třech umístěních: webový server, všechny proxy servery a webový prohlížeč. Můžete ovládat, přesně, kde je obsah ukládána úpravou vlastnosti umístění atributu [OutputCache].
...
Všimněte si, že atribut [OutputCache] výpis 4 obsahuje vlastnost umístění nastaven na hodnotu OutputCacheLocation.Client. Atribut [OutputCache] také obsahuje vlastnost NoStore. Vlastnost NoStore slouží k informování proxy servery a prohlížeče, by neměl uložených trvalá kopie obsahu v mezipaměti.
...
Tento problém můžete vyřešit využívat výhod vlastnost VaryByParam atributu [OutputCache]. Tato vlastnost umožňuje vytvářet různé verze uložené v mezipaměti velmi stejný obsah, pokud parametr formuláře nebo parametr řetězce dotazu se liší.
...
Řadič v výpis 7 znázorňuje, jak můžete použít profil Cache1Hour pro akce kontroleru s atributem [OutputCache].
