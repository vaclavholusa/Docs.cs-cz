...
---
<a name="understanding-and-handling-connection-lifetime-events-in-signalr"></a>Princip a zpracování událostí životnost připojení SignalR
====================
...
> 
> Článek předpokládá, že již máte některé informace o události životního cyklu SignalR a připojení. Úvod do SignalR, najdete v části [Úvod do SignalR](../getting-started/introduction-to-signalr.md). Seznam událostí životnost připojení najdete v následujících zdrojích informací:
> 
> - [Postupy: zpracování události životnost připojení v třídy rozbočovače](hubs-api-guide-server.md#connectionlifetime)
> - [Postupy: zpracování události životnost připojení v klientech JavaScript](hubs-api-guide-javascript-client.md#connectionlifetime)
> - [Postupy: zpracování události životnost připojení v klientů .NET](hubs-api-guide-net-client.md#connectionlifetime)
> 
...
- [Zjišťování důvod odpojení](#detectingreasonfordisconnection)
...
## <a name="detecting-the-reason-for-a-disconnection"></a>Zjišťování důvod odpojení

SignalR 2.1 přidá přetížení server `OnDisconnect` událost, která označuje, pokud klient úmyslně odpojen spíše než vyprší časový limit. `StopCalled` Parametr je hodnota true, pokud klient explicitně zavřel připojení. V jazyce JavaScript, pokud k serverové chybě vedla klientovi odpojit, informace o chybě předány klientovi jako `$.connection.hub.lastError`.

**Serverový kód C#: `stopCalled` parametr**
