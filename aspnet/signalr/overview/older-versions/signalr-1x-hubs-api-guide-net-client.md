...
ms.locfileid: "28037477"
---
<a name="aspnet-signalr-hubs-api-guide---net-client-signalr-1x"></a>Funkce SignalR technologie ASP.NET centra API Průvodce – klient .NET (SignalR 1.x)
====================
...
- [Rozhraní API Průvodce pro rozbočovače SignalR – JavaScript klienta](../guide-to-the-api/hubs-api-guide-javascript-client.md)
...
Pokud je odlišná od verze, ke které máte na serveru verze funkce signalr, zda máte na straně klienta, je často moci přizpůsobit rozdílu SignalR. Například při vydání SignalR verze 2.0 a která nainstalujte na server, server podporovat klienty, kteří mají 1.1.x instalován, a klienti, kteří mají nainstalovanou 2.0. Pokud je rozdíl mezi verze na serveru a verzí v klientovi příliš velký, vyvolá SignalR `InvalidOperationException` výjimky, když se klient pokusí navázat připojení. Chybová zpráva "`You are using a version of the client that isn't compatible with the server. Client version X.X, server version X.X`".
...
`HubConnection` Třída je bezpečné pro přístup z více vláken.
...
Objekt proxy serveru je bezpečné pro přístup z více vláken. V faktu, když zavoláte `HubConnection.CreateHubProxy` víckrát se stejným `hubName`, stejné uložené v mezipaměti získáte `IHubProxy` objektu.
...
Další informace najdete v tématu [pochopení a zpracování události životnost připojení v systému SignalR](../guide-to-the-api/handling-connection-lifetime-events.md).
