...
---
<a name="determining-what-files-need-to-be-deployed-vb"></a>Zjištění, jaké soubory musí mít nasadit (VB)
====================
...
Aby modul ASP.NET ke zpracování žádosti pro tuto stránku, na stránce kódu část ( *`WebPage`* `.aspx.vb` souboru) musí být nejprve zkompilovány. Tato kompilace může dojít, explicitně nebo automaticky.
...
Společnost Microsoft vydala Visual Studio 2005 vyřadit podpora modelu projektu webové aplikace a nahradí modelem projektu webové stránky. Model projektu webu rozlišené sama sebe z: *projektu webové aplikace* modelů následujícími způsoby:
...
**Obrázek 1**: The kontroly knihy web nabízí recenzí na celé řady různých doprovodných názvy ([kliknutím ji zobrazíte obrázek v plné velikosti](determining-what-files-need-to-be-deployed-vb/_static/image3.png))
...
Model projektu webové aplikace používá explicitní kompilace – projektu zdrojový kód je zkompilován do jednoho sestavení při každém sestavení aplikace. Tato kompilace obsahuje soubory kódu na pozadí na stránkách ASP.NET (~ /`Default.aspx.vb`, ~ /`About.aspx.vb`, a tak dále), stejně jako `BasePage.vb` třídy. Výsledné sestavení je pojmenováno `BookReviewsWAP.dll` a nachází se ve vaší aplikaci `Bin` adresáře.
...
Kopírování souborů se zdrojovým kódem na stránkách ASP.NET k produkčnímu prostředí nepotřebujete, ani je potřeba zkopírovat `BasePage.vb` souboru třídy.
