---
uid: web-forms/overview/presenting-and-managing-data/model-binding/index
title: "Model webových formulářů v sadě Visual Studio 2013 a vazeb | Microsoft Docs"
author: rick-anderson
description: "Jak pracovat s daty pomocí vazby modelu a webové formuláře."
ms.author: aspnetcontent
manager: wpickett
ms.date: 02/13/2013
ms.topic: article
ms.assetid: b94c8eca-9cde-417e-a8bd-e0ff53600984
ms.technology: dotnet-webforms
ms.prod: .net-framework
msc.legacyurl: /web-forms/overview/presenting-and-managing-data/model-binding
msc.type: chapter
ms.openlocfilehash: f653a1394af8bfc562c244dad00ff35bc4744647
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 11/10/2017
---
<a name="model-binding-and-web-forms-in-visual-studio-2013"></a>Vazby modelu a webové formuláře v sadě Visual Studio 2013
====================
> Jak pracovat s daty pomocí vazby modelu a webové formuláře.


- [Načítání a zobrazení dat](retrieving-data.md)
- [Aktualizace, odstraňování a vytváření dat](updating-deleting-and-creating-data.md)
- [Řazení, stránkování a filtrování dat](sorting-paging-and-filtering-data.md)
- [Ovládací prvek JQuery UI Datepicker integrace](integrating-jquery-ui.md)
- [Filtrování dat pomocí hodnoty řetězců dotazu](using-query-string-values-to-retrieve-data.md)
- [Přidání vrstvu obchodní logiky](adding-business-logic-layer.md)
