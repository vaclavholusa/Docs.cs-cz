...
Pokud jsou služby řešeny pomocí `ActivatorUtilities`, vkládání pomocí konstruktoru vyžaduje existenci jediného použitelného konstruktoru. Přetížení konstruktoru je podporováno, ale může existovat pouze jedno přetížením, jehož argumenty jsou splnitelné vkládáním závislostí.
...
* Pokud jsou služby s přechodnou životností vytvořeny při vyžádání, `OperationsId` služby `IOperationTransient` se bude lišit od `OperationsId` služby `OperationService`. `OperationService` obdrží novou instanci třídy `IOperationTransient`. Nová instance implikuje rozdílné `OperationsId`.
* Pokud jsou služby s vymezenou životností vytvořeny při požadavku, `OperationsId` ze služby `IOperationScoped` je stejné jako u `OperationService` v rámci požadavku. Napříč různými požadavky obě služby sdílejí jinou hodnotu `OperationsId`.
* Pokud jsou singletony a služby se životností typu singleton vytvářeny jednou napříč všemi požadavky a službami, `OperationsId` je konstantní mezi všemi požadavky služeb.
...
Při práci s vkládáním závislostí mejte na paměti následující doporučení:

* Vyhněte se ukládání dat a konfigurace přímo do kontejneru služeb. Například nákupní košík uživatele by neměl být přidáván do kontejneru služeb. Konfigurace by měla používat [vzor možností (options pattern)](xref:fundamentals/configuration/options). Podobně se vystříhejte použití objektů "držící data", jejichž jediným účelem je poskytnutí přístupu k některému jinému objektu. Je lepší požádat o skutečné položky přímo pomocí vkládání závislostí, pokud je to možné.
...
* Vyhněte se použití vzoru Service Locator (Service Location pattern, například [IServiceProvider.GetService](/dotnet/api/system.iserviceprovider.getservice)).
...
Vkládání závislostí je *alternativou* ke vzorům statického/globálního přístupu k objektům. Pokud budete kombinovat tento přístup se statickým přístupem k objektu, nebudete moci plně využít všech výhod vkládání závislostí.
