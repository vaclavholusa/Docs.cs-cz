...
Kestrel je napříč platformami [webového serveru pro ASP.NET Core](xref:fundamentals/servers/index) na základě [libuv](https://github.com/libuv/libuv), knihovny a platformy asynchronní vstupně-výstupní operace. Kestrel je webový server, který je zahrnut ve výchozím nastavení v šablony projektů ASP.NET Core.
...
Doporučujeme používat Kestrel s reverzní proxy server, pokud Kestrel je dostupná jenom v případě k interní síti.
...
* `serverOptions.Configure(context.Configuration.GetSection("Kestrel"))` Vrátí `KestrelConfigurationLoader` s `.Endpoint(string name, options => { })` metoda, která umožňuje doplnit nakonfigurovaná nastavení pro koncový bod:
  ...
  Můžete rovněž přímo přistupovat ke `KestrelServerOptions.ConfigurationLoader` zachovat procházení na existující zavaděč, jako je třeba poskytované `WebHost.CreatedDeafaultBuilder`.
...
Všimněte si, jak tento příklad konfiguruje SSL pro koncový bod konkrétní pomocí [ListenOptions](/dotnet/api/microsoft.aspnetcore.server.kestrel.core.listenoptions). Konfigurovat další nastavení Kestrel pro specifické koncové body pomocí stejné rozhraní API.
...
* Použití middleware pro filtrování požadavků pomocí `Host` záhlaví. Následuje ukázka middleware:
...
*appSettings. Development.JSON* (localhost konfigurační soubor):
