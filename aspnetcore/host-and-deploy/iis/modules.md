...
ms.locfileid: "43312343"
---
# <a name="iis-modules-with-aspnet-core"></a>Moduly IIS s ASP.NET Core

Podle [Luke Latham](https://github.com/guardrex)

Aplikace ASP.NET Core jsou hostované službou IIS v konfigurace reverzního proxy serveru. Některé z nativních modulů IIS a všechny moduly služby IIS spravované nejsou k dispozici pro zpracování žádosti o aplikace ASP.NET Core. V mnoha případech ASP.NET Core nabízí alternativu k funkcím nativní a spravované moduly služby IIS.
...
Pokud se vyžadují pro odebrání modulu s nastavením v *web.config*odemknout modulu a odemknutí  **\<moduly >** část *web.config* první:
...
2. Nasazení aplikace bez  **\<moduly >** tématu *web.config*. Pokud je aplikace nasazená s *web.config* obsahující  **\<moduly >** vyvolá výjimku, část bez nutnosti odemknout části nejprve v Správce služby IIS, Configuration Manager Při pokusu o odemknutí části. Proto nasadit aplikaci bez  **\<moduly >** oddílu.

3. Odemknout  **\<moduly >** část *web.config*. V **připojení** bočním panelu vyberte web v **lokality**. V **správu** oblasti, otevřete **Editor konfigurace**. Použít ovládací prvky navigace a vyberte `system.webServer/modules` oddílu. V **akce** bočního panelu na pravé straně vyberte **odemknout** části.

4. V tomto okamžiku  **\<moduly >** části mohou být přidány do *web.config* soubor s  **\<odebrat >** prvek, který chcete odebrat z modulu aplikace. Více  **\<odebrat >** elementy lze přidat více modulů odebrat. Pokud *web.config* změn na serveru, okamžitě provést stejné změny do projektu *web.config* soubor místně. Odebrání modulu díky tomu nebude mít vliv na používání modulu s jinými aplikacemi na serveru.
