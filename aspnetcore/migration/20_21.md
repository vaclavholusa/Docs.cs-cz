...
* Odkazování na projekty `Microsoft.AspNetCore.All` nebo `Microsoft.AspNetCore.App` balíčky musíte nastavit SDK projektu `Microsoft.NET.Sdk.Web` v horní části souboru projektu (`<Project Sdk="Microsoft.NET.Sdk.Web">`).
* Projekty odkazy na balíčky nebo projekty, které odkazují přechodně `Microsoft.AspNetCore.All` nebo `Microsoft.AspNetCore.App`:
  * Musíte nastavit SDK projektu `Microsoft.NET.Sdk.Web` v horní části souboru projektu (`<Project Sdk="Microsoft.NET.Sdk.Web">`).
  * Musí odkazovat na stejné sdílené runtime balíčku. Pokud odkazuje LibraryA `Microsoft.AspNetCore.App`, musí také odkazovat všechny projekty odkazující na LibraryA `Microsoft.AspNetCore.App`.
* Spustitelný soubor projektů (projekty, které obsahují aplikace, které jsou spouštěny s `dotnet run` nebo aplikace, které spustit a otestovat projekty pro aplikace) nesmí zadejte verzi pro `Microsoft.AspNetCore.App`. Určuje verzi implicitně prostřednictvím sady SDK `<PackageReference Include="Microsoft.AspNetCore.App" />`.
* Odkazovaný projektů (projekty, které nejsou vstupní bod a odkazy na projekt `Microsoft.AspNetCore.All` nebo `Microsoft.AspNetCore.App`) musíte zadat verze balíčku.
