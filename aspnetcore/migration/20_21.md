...
Aplikace, které mají kód pod identitou a nahraďte 2.0 Identity uživatelského rozhraní pomocí 2.1 Identity knihovny potřeba vzít v úvahu adresy URL Identity mají `/Identity` segment pro identifikátory URI. Jeden ze způsobů, jak zpracovávat nové Identity koncové body jsou nastavení přesměrování, třeba z `/Account/Login` k `/Identity/Account/Login`.
...
* Pomocí Identity uživatelského rozhraní 2.0 kódu beze změny. Pomocí Identity uživatelského rozhraní 2.0 kódu se plně podporuje. To je dobrý přístup, když nejvýznamnější změny se provedly pro vygenerovaný kód Identity.
...
* Odstranit generátor Identity vygeneruje `IdentityDbContext` derrived třídy v *oblasti/Identity/Data/* složky.
...
* Aktualizace *_LoginPartial.cshtml* souboru:
  ...
  * Přidat `asp-area="Identity"` formuláře a kotevní odkazy.
