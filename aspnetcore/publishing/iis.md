...
*Web.config* souboru především konfiguruje modul základní technologie ASP.NET. Volitelně ji mohou zadat další nastavení konfigurace služby IIS. Vytváření, transformace a publikování *web.config* zpracováván pomocí .NET SDK webového jádra (`Microsoft.NET.Sdk.Web`). Sada SDK je nastavena v horní části souboru projektu (*.csproj*), `<Project Sdk="Microsoft.NET.Sdk.Web">`. Zabránit transformace sady SDK *web.config* soubor, přidejte  **\<IsTransformWebConfigDisabled >** vlastnost k souboru projektu s nastavením `true`:
